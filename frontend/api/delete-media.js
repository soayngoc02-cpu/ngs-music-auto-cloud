...
async function cleanupMusicDna(audioKey) {
  const listed = await client().send(new ListObjectsV2Command({ Bucket: bucket(), Prefix: 'music/dna/', MaxKeys: 1000 }));
  const matches = [];
  ...
  if (matches.length) {
    await client().send(new DeleteObjectsCommand({ Bucket: bucket(), Delete: { Objects: matches, Quiet: true } }));
  }
  return matches.map((item) => item.Key);
}

async function deleteRenderedVideo(body) {
  ...
  if (recordKey && !recordKey.startsWith('jobs/history/') && !recordKey.startsWith('jobs/done/')) throw new Error('Invalid video record key');
  ...
  if (jobId) {
    ...
    if (latest && String(latest.output_key || '') === outputKey && latestKey !== recordKey) objects.push({ Key: latestKey });
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (kind === 'video') {
      const deleted = await deleteRenderedVideo(body);
      return res.status(200).json({ ok: true, kind, deleted });
    }

    const prefix = PREFIXES[kind];
    const key = String(body.key || '').trim();
    if (!prefix || !key.startsWith(prefix)) return res.status(400).json({ error: 'Invalid media kind or key' });

    await client().send(new DeleteObjectCommand({ Bucket: bucket(), Key: key }));
    const deleted = [key];
    if (kind === 'music') deleted.push(...await cleanupMusicDna(key));
    return res.status(200).json({ ok: true, kind, deleted });
  } catch (err) {

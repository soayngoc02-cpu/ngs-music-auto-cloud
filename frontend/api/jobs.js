...
const VISUAL_EFFECTS = new Set(['none','zoom_in','zoom_out','pan_left','pan_right','pan_up','pan_down','drift','pulse','ken_burns','cinematic','dreamy','soft_glow','vignette','film_grain','warm_film','cool_night','vintage','lofi','dramatic','monochrome','dynamic_mix']);
const SUBTITLE_STYLES = new Set(['clean_pro','tiktok_pop','neon_glow','cinema','glass_box','gold','heavy_outline','minimal']);
...
async function triggerWorkflow(jobKey) {
  ...
  const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/actions/workflows/render-job.yml/dispatches`, {
    ...
    headers: {
      ...
      'X-GitHub-Api-Version': '2022-11-28', 'User-Agent': 'ngs-music-studio-vercel', 'Content-Type': 'application/json',
    },
...
function normalizeMediaItems(body) {
  const raw = Array.isArray(body.media_items) ? body.media_items.slice(0, 40) : [];
  ...
  })).filter((item) => item.key && ['image', 'video'].includes(item.type));
  if (!items.length && body.image_key) items.push({ key: String(body.image_key), type: 'image', duration_sec: 0, start_sec: 0 });
  return items;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!mediaItems.length) return res.status(400).json({ error: 'At least one image or video is required' });

    const aspect = String(body.aspect_ratio || '9:16');
    const quality = String(body.quality || '1080').toLowerCase();
    if (!PRESETS[aspect] || !PRESETS[aspect][quality]) return res.status(400).json({ error: 'Invalid aspect_ratio or quality' });
    const [width, height] = PRESETS[aspect][quality];

    const musicMode = String(body.music_mode || (body.audio_key ? 'file' : 'auto')).toLowerCase();
    if (!['auto', 'file', 'manual'].includes(musicMode)) return res.status(400).json({ error: 'Invalid music_mode' });
    const audioStartSec = Number(body.audio_start_sec || 0);
    const durationSec = Number(body.duration_sec || 0);
    if (!Number.isFinite(audioStartSec) || audioStartSec < 0) return res.status(400).json({ error: 'Invalid audio_start_sec' });
    if (!Number.isFinite(durationSec) || durationSec < 0) return res.status(400).json({ error: 'Invalid duration_sec' });
    if (musicMode === 'manual' && durationSec <= 0) return res.status(400).json({ error: 'Manual trim requires duration_sec > 0' });
    ...
    if (effectMode === 'manual' && !VISUAL_EFFECTS.has(effectPreset)) return res.status(400).json({ error: `Invalid visual effect: ${effectPreset}` });
    ...
    const job = {
      job_id: jobId, render_id: renderId,
      image_key: mediaItems[0].key,
      media_items: mediaItems,
      audio_key: String(body.audio_key || ''), output_key: outputKey,
      music_mode: musicMode, audio_start_sec: audioStartSec, audio_end_sec: Number(body.audio_end_sec || 0),
      aspect_ratio: aspect, quality, fps: Number(body.fps || 30), duration_sec: durationSec,
      lyrics: { source: String(lyrics.source || 'auto'), key: String(lyrics.key || ''), text: String(lyrics.text || ''), use_for_analysis: lyrics.use_for_analysis !== false, render: subtitleEnabled },
      visual_effect: { mode: effectMode, preset: effectPreset, intensity: effectIntensity },
      subtitle: {
        enabled: subtitleEnabled, style: subtitleStyle, animation: subtitleAnimation, position: subtitlePosition,
        size: subtitleSize, max_lines: subtitleMaxLines, sync_mode: subtitleSyncMode, language: subtitleLanguage,
        model: subtitleModel, min_confidence: subtitleMinConfidence, font_size: subtitleFontSize,
        y_percent: subtitleYPercent, safe_width_percent: subtitleSafeWidthPercent,
      },
      requested_width: width, requested_height: height, status: 'pending', created_at: now.toISOString(),
    };

    await removeOldStatus(jobId);
    const jobKey = `jobs/pending/${jobId}.json`;
    await client().send(new PutObjectCommand({ Bucket: bucket(), Key: jobKey, Body: JSON.stringify(job, null, 2), ContentType: 'application/json; charset=utf-8', CacheControl: 'no-store' }));
    ...
    try {
      ...
      await client().send(new PutObjectCommand({ Bucket: bucket(), Key: `jobs/failed/${jobId}.json`, Body: JSON.stringify(failed, null, 2), ContentType: 'application/json; charset=utf-8', CacheControl: 'no-store' }));

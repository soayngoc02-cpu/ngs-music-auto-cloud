...
export async function getCopilotAccessToken(){const token=await getSetting('copilot_token');if(!token)throw new Error('Chưa kết nối GitHub Copilot');const exp=Number((await getSetting('copilot_token_expires_at'))||0);if(!exp||Date.now()<exp-5*60*1000)return token;return refreshCopilotToken()}
...
export async function testCopilotAccess(){
  ...
  try{await client.start();const models=await client.listModels();await markCopilot('ok','');return{ok:true,count:Array.isArray(models)?models.length:0,models:(Array.isArray(models)?models:[]).slice(0,12).map(x=>x.id||x.name||x.model||'').filter(Boolean)}}
  catch(e){await markCopilot(isPolicyError(e)?'blocked':'error',errText(e));throw e}
...
export async function copilotPlan(project,recent,music){
  ...
  try{await client.start();session=await client.createSession({model,sessionId:`ngs-${project.id}-${Date.now()}`});const ev=await session.sendAndWait({prompt:planPrompt(project,recent,music)},120000);if(!ev)throw new Error('Copilot không phản hồi');await markCopilot('ok','');return stripJson(ev.data.content)}
  catch(e){await markCopilot(isPolicyError(e)?'blocked':'error',errText(e));throw e}
...
export async function planWithFallback(project,recent,music){
  const errors=[];const hasCopilot=!!(await getSetting('copilot_token').catch(()=>''));
  if(hasCopilot){try{return{plan:await copilotPlan(project,recent,music),provider:'copilot'}}catch(e){errors.push(`Copilot: ${errText(e)}`)}}else errors.push('Copilot: chưa kết nối');
  const premium=String(await getSetting('premium_fallback').catch(()=>''))==='true';if(!premium)throw new Error(`${errors.join(' | ')}. Premium fallback đang TẮT.`);
...
export async function saveGeneratedImage(ownerId,projectId,index,prompt){
  const errors=[];let img;try{img=await cloudflareImage(prompt)}catch(e){errors.push(errText(e));const premium=String(await getSetting('premium_fallback').catch(()=>''))==='true';if(!premium)throw e;if(await getSetting('gemini_api_key').catch(()=>'')){try{img=await geminiImage(prompt)}catch(x){errors.push(errText(x))}}if(!img&&await getSetting('openai_api_key').catch(()=>'')){try{img=await openaiImage(prompt)}catch(x){errors.push(errText(x))}}if(!img)throw new Error(`Không tạo được ảnh: ${errors.join(' | ')}`)}
  const mid=newid(),ext=img.mime.includes('png')?'png':'jpg';await q('insert into media(id,owner_id,kind,name,mime,data,meta,shared,created_at) values($1,$2,$3,$4,$5,$6,$7,true,now())',[mid,ownerId,'image',`${projectId}-scene-${index+1}.${ext}`,img.mime,img.buffer,{generatedBy:img.provider,projectId,prompt,index}]);return{id:mid,provider:img.provider}
}

export async function testPremiumProvider(provider){
  if(provider==='gemini'){const key=await getSetting('gemini_api_key');if(!key)throw new Error('Chưa lưu Gemini API key');const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models',{headers:{'x-goog-api-key':key}});if(!r.ok)throw new Error(`Gemini key lỗi ${r.status}: ${(await r.text()).slice(0,220)}`);return{ok:true}}
  if(provider==='openai'){const key=await getSetting('openai_api_key');if(!key)throw new Error('Chưa lưu OpenAI API key');const r=await fetch('https://api.openai.com/v1/models',{headers:{Authorization:`Bearer ${key}`}});if(!r.ok)throw new Error(`OpenAI key lỗi ${r.status}: ${(await r.text()).slice(0,220)}`);return{ok:true}}

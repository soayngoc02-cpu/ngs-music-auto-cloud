...
const n=Number((await q('select count(*)::int c from users')).rows[0].c);if(!n)await q('insert into users(id,email,password_hash,salt,role,active,created_at,updated_at) values($1,$2,$3,$4,$5,true,now(),now())',[newid(),process.env.OWNER_EMAIL||'owner@ngs.local',process.env.OWNER_PASSWORD_HASH||'cdce21de879f6b8ed0ba83e9e5f66623fe77a53be7cf04487b71dd44494769a9067576ccc92b397e6dff807f4890fd2a872bebcbcf57738711a765dc40ca0eda',process.env.OWNER_PASSWORD_SALT||'a3532f80e15b079b27c1e83e96ebd3dc','SUPER_ADMIN'])}
